...
export function resolveSyntaxColor(
	...
	if (parts.length !== 2) {
		throw new Error(
			`Invalid syntax role path: "${dotPath}". Expected "group.role" format.`,
		);
	}
...
export function loadFontStyles(
	paletteDir: string,
): Record<string, FontStyleMapping> {
	const filePath = path.join(paletteDir, "syntax.json");
	const raw = fs.readFileSync(filePath, "utf-8");
	const mapping = JSON.parse(raw) as SyntaxMapping & {

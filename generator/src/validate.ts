...
import { loadSyntaxMapping, loadScopes } from "./syntaxMapping";
    ...
    try {
      ...
      for (const name of colorNames) {
        ...
        if (color.hex.toUpperCase() !== expectedHex) {
          console.error(`  Hex/RGB mismatch for ${name}: hex=${color.hex}, expected=${expectedHex}`);
          errors++;
        }
      }
      if (errors > 0) {
        ...
        console.log(`  OK — ${palette.info.name}: ${colorNames.length} colors validated`);
      }
    } catch (e) {
...
function validateMappings(): boolean {
  ...
  for (const variant of VARIANTS) {
    ...
    const groups = ["background", "surface", "text", "border", "editor", "terminal", "status", "vcs"] as const;
    for (const group of groups) {
      const groupMap = (uiMapping as Record<string, Record<string, string>>)[group];
      for (const [key, colorName] of Object.entries(groupMap)) {
        if (!(colorName in palette.colors)) {
          console.error(`  Unknown color "${colorName}" in ui-mapping.${group}.${key} for ${variant}`);
          ok = false;
        }
      }
    }

    const syntaxGroups = ["keywords", "storage", "literals", "comments", "functions", "variables", "entities", "support", "invalid", "emphasis"] as const;
    for (const group of syntaxGroups) {
      const groupMap = (syntaxMapping as Record<string, Record<string, string>>)[group];
      if (!groupMap) continue;
      for (const [key, colorName] of Object.entries(groupMap)) {
        if (!(colorName in palette.colors)) {
          console.error(`  Unknown color "${colorName}" in syntax.${group}.${key} for ${variant}`);
          ok = false;
        }
      }
    }
...
function validateScopes(): boolean {
  ...
  for (const entry of scopes) {
    ...
    if (!groupMap || !(role in groupMap)) {
      console.error(`  Unknown syntaxRole "${entry.syntaxRole}" in scope "${entry.name}"`);
      ok = false;
    }
  }

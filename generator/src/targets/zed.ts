import type { Palette, UIMapping, SyntaxMapping, FontStyleMapping } from "../types";
...
function hex8(palette: Palette, colorName: string, alpha: number = 1): string {
  ...
  const a = Math.round(alpha * 255).toString(16).padStart(2, "0").toLowerCase();
  return `#${color.hex}${a}`;
}
...
function bright8(palette: Palette, colorName: string, factor: number = 0.2, alpha: number = 1): string {
  const color = resolveColor(palette, colorName);
  const brightRgb = adjustBrightness(color.rgb, factor, "lighten");
  return hex8rgb(brightRgb, alpha);
}

function dim8(palette: Palette, colorName: string, factor: number = 0.3, alpha: number = 1): string {
  const color = resolveColor(palette, colorName);
...
function buildUIStyle(palette: Palette, isDark: boolean): Record<string, string | null> {
  const s = (colorName: string, alpha?: number) => hex8(palette, colorName, alpha);
  ...
  const result: Record<string, string | null> = {
    ...
    "editor.document_highlight.read_background": isDark ? s("flamingo", 0.1) : s("teal", 0.1),
    ...
    "terminal.ansi.dim_black": isDark ? dim8(palette, "text", 0.3) : dim8(palette, "text", 0.3),
...
function buildSyntax(
  ...
  syntax["selector.pseudo"] = isDark ? syn("lavender", palette) : syn("blue", palette);
  ...
  syntax["constructor"] = isDark ? syn("lavender", palette) : syn("blue", palette);
...
function buildPlayers(palette: Palette): ZedPlayer[] {
  const playerColors = ["teal", "flamingo", "sky", "lavender", "green", "red", "yellow", "peach"];
...
export function generateZedTheme(
  ...
  const darkStyle = buildZedStyle(darkPalette, true, uiMapping, syntaxMapping, fontStyles);
  const lightStyle = buildZedStyle(lightPalette, false, uiMapping, syntaxMapping, fontStyles);

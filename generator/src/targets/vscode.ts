...
export function generateVSCodeTheme(
  ...
  colors["selection.background"] = uc(uiMapping, palette, "editor", "selection", OPACITY_RULES.selection);
  ...
  colors["editorLineNumber.foreground"] = uc(uiMapping, palette, "editor", "line_number");
  colors["editorLineNumber.activeForeground"] = uc(uiMapping, palette, "editor", "active_line_number");
  colors["editor.lineHighlightBackground"] = uc(uiMapping, palette, "editor", "active_line_background", OPACITY_RULES.active_line);
  colors["editor.selectionBackground"] = uc(uiMapping, palette, "editor", "selection", OPACITY_RULES.selection);
  colors["editor.inactiveSelectionBackground"] = uc(uiMapping, palette, "editor", "selection", OPACITY_RULES.inactive_selection);
  colors["editorCursor.foreground"] = uc(uiMapping, palette, "editor", "cursor");
  colors["editor.findMatchBackground"] = uc(uiMapping, palette, "editor", "find_match", OPACITY_RULES.search_match);
  colors["editor.findMatchHighlightBackground"] = uc(uiMapping, palette, "editor", "find_match", OPACITY_RULES.search_match_border);
  colors["editor.findMatchBorder"] = uc(uiMapping, palette, "editor", "find_active_match");
  ...
  colors["editorGutter.deletedBackground"] = uc(uiMapping, palette, "vcs", "deleted", 0x80 / 255);

  colors["editorWhitespace.foreground"] = uc(uiMapping, palette, "text", "tertiary", 0x40 / 255);
  colors["editorIndentGuide.background1"] = uc(uiMapping, palette, "border", "default", 0x50 / 255);
  colors["editorIndentGuide.activeBackground1"] = uc(uiMapping, palette, "text", "tertiary", 0x80 / 255);

  colors["editorError.foreground"] = uc(uiMapping, palette, "status", "error");
  colors["editorWarning.foreground"] = uc(uiMapping, palette, "status", "warning");
  colors["editorInfo.foreground"] = uc(uiMapping, palette, "status", "info");
  colors["editorHint.foreground"] = pc(palette, "sapphire");

  colors["activityBar.background"] = uc(uiMapping, palette, "background", "elevated");
  colors["activityBar.foreground"] = uc(uiMapping, palette, "text", "secondary");
  colors["activityBar.inactiveForeground"] = uc(uiMapping, palette, "text", "tertiary");
  colors["activityBar.border"] = uc(uiMapping, palette, "border", "default");
  colors["activityBarBadge.background"] = uc(uiMapping, palette, "editor", "cursor");
  colors["activityBarBadge.foreground"] = uc(uiMapping, palette, "text", "on_accent");

  colors["sideBar.background"] = uc(uiMapping, palette, "background", "elevated");
  colors["sideBar.foreground"] = uc(uiMapping, palette, "text", "primary");
  colors["sideBar.border"] = uc(uiMapping, palette, "border", "default");
  colors["sideBarSectionHeader.background"] = uc(uiMapping, palette, "surface", "active");
  colors["sideBarSectionHeader.foreground"] = uc(uiMapping, palette, "text", "secondary");

  colors["tab.activeBackground"] = uc(uiMapping, palette, "editor", "background");
  colors["tab.inactiveBackground"] = uc(uiMapping, palette, "background", "elevated");
  ...
  colors["editorGroupHeader.tabsBackground"] = uc(uiMapping, palette, "background", "elevated");
  colors["editorGroupHeader.tabsBorder"] = uc(uiMapping, palette, "border", "default");

  colors["titleBar.activeBackground"] = uc(uiMapping, palette, "background", "elevated");
  colors["titleBar.activeForeground"] = uc(uiMapping, palette, "text", "primary");
  colors["titleBar.inactiveBackground"] = uc(uiMapping, palette, "background", "deepest");
  colors["titleBar.inactiveForeground"] = uc(uiMapping, palette, "text", "tertiary");
  colors["titleBar.border"] = uc(uiMapping, palette, "border", "default");

  colors["statusBar.background"] = uc(uiMapping, palette, "background", "deepest");
  ...
  colors["panelTitle.activeForeground"] = uc(uiMapping, palette, "text", "primary");
  colors["panelTitle.inactiveForeground"] = uc(uiMapping, palette, "text", "tertiary");

  colors["scrollbarSlider.background"] = uc(uiMapping, palette, "text", "tertiary", 0x4C / 255);
  colors["scrollbarSlider.hoverBackground"] = uc(uiMapping, palette, "text", "tertiary", 0x80 / 255);
  colors["scrollbarSlider.activeBackground"] = uc(uiMapping, palette, "text", "secondary", 0xB0 / 255);

  colors["terminal.background"] = uc(uiMapping, palette, "editor", "background");
  ...
  colors["terminal.ansiMagenta"] = uc(uiMapping, palette, "terminal", "magenta");
  colors["terminal.ansiCyan"] = uc(uiMapping, palette, "terminal", "cyan");
  colors["terminal.ansiWhite"] = uc(uiMapping, palette, "terminal", "white");
  colors["terminal.ansiBrightBlack"] = uc(uiMapping, palette, "terminal", "bright_black");
  colors["terminal.ansiBrightRed"] = bright(palette, uiMapping.terminal.red);
  colors["terminal.ansiBrightGreen"] = bright(palette, uiMapping.terminal.green);
  colors["terminal.ansiBrightYellow"] = bright(palette, uiMapping.terminal.yellow);
  colors["terminal.ansiBrightBlue"] = bright(palette, uiMapping.terminal.blue);
  colors["terminal.ansiBrightMagenta"] = bright(palette, uiMapping.terminal.magenta);
  colors["terminal.ansiBrightCyan"] = bright(palette, uiMapping.terminal.cyan);
  colors["terminal.ansiBrightWhite"] = bright(palette, uiMapping.terminal.white);
  ...
  colors["input.placeholderForeground"] = uc(uiMapping, palette, "text", "tertiary");
  ...
  colors["minimap.findMatchHighlight"] = uc(uiMapping, palette, "editor", "find_match");
  colors["minimap.selectionHighlight"] = uc(uiMapping, palette, "editor", "selection", OPACITY_RULES.selection);
  ...
  const tokenColors: VSCodeTokenColor[] = scopes.map((entry) => {
    ...
    if (syntaxGroup && syntaxGroup[parts[1]]) {
      settings.foreground = resolveSyntaxColor(syntaxMapping, palette, entry.syntaxRole);
    } else {
      settings.foreground = resolveUIColor(uiMapping, palette, parts[0], parts[1]);
    }
